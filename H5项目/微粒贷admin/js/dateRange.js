...
pickerDateRange.prototype.addCompareDate = function(date){
	if(this.multiList.indexOf(date) == -1){
		this.multiList.push(date);
		var d = date.toDate();
		$('#'+ this.calendarId + '_'  + (d.getFullYear() + '-' + (d.getMonth() + 1) + '-' + d.getDate())).attr('class', (this.mOpts.theme == 'ta' ? this.mOpts.clickCss  : this.mOpts.theme + '_' + this.mOpts.selectCss));
	}
}

pickerDateRange.prototype.removeCompareCSS = function(){
	for(var i=0;i<this.multiList.length;i++){
		var d = this.multiList[i].toDate();
		$('#'+ this.calendarId + '_'  + (d.getFullYear() + '-' + (d.getMonth() + 1) + '-' + d.getDate())).removeClass((this.mOpts.theme == 'ta' ? this.mOpts.clickCss  : this.mOpts.theme + '_' + this.mOpts.selectCss));
	}
}
...
pickerDateRange.prototype.selectDate = function(ymd) {
    ...
    if(this.startDateId == this.dateInput) {
        ...
        // 为当前点加样式 -- 如果不是是选择模式
		var _cls = (this.mOpts.theme == 'ta' ? this.mOpts.clickCss  : this.mOpts.theme + '_' + this.mOpts.selectCss);
		if(this.mOpts.multiMode){
			if(this.multiList.indexOf(ymdFormat) != -1){
				this.multiList.remove(ymdFormat);
				$('#'+ this.calendarId + '_'  + ymd).attr('class', '');
			}else{
				if(this.multiList.length<this.mOpts.multiLimit){
					this.multiList.push(ymdFormat);
					$('#'+ this.calendarId + '_'  + ymd).attr('class', _cls);
				}
			}
		}else{
			// 移除样式
...
pickerDateRange.prototype.close = function(btnSubmit) {
	...
	if(btnSubmit){
		...
		if(input && input.tagName == 'INPUT'){
			...
			if(this.mOpts.defaultText){
				$('#' + this.inputId).html(this.mOpts.defaultText);
			}else{
...
pickerDateRange.prototype.fillDate = function(year, month, index) {
    ...
    for(var d = dateBegin; d.getTime() <= dateEnd.getTime(); d.setDate(d.getDate() + 1)) {
        if(d.getTime() < firstDayOfMonth.getTime()) { // 当前月之前的日期
            ...
            if(this.mOpts.multiMode){
            	//设置默认选择日期
                for(var _i=0;_i<this.multiList.length;_i++){
                	if(d.getTime() == this.multiList[_i].toDate().getTime()){
                		tdClass = (this.mOpts.theme == 'ta' ? this.mOpts.clickCss  : this.mOpts.theme + '_' + this.mOpts.selectCss);
                	}
    	        }
            }
